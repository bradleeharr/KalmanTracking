...
def kalman_tracking(annotations, model='Velocity', plot=True):
    ...
    for obj_id in obj_ids:
        ...
        filtered_mses[obj_id] = np.mean((imputed_measurements[obj_id] - filtered_state_means[obj_id][:, :2]) ** 2)
        smoothed_mses[obj_id] = np.mean((imputed_measurements[obj_id] - smoothed_state_means[obj_id][:, :2]) ** 2)
