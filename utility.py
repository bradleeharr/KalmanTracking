    ...
    with open(file_path, "r") as csvfile:
        ...
        for row in reader:
            (
                ...
                occluded,
                noisy_frame,
            ) = [float(val) if val else 0 for val in row]
            annotation = (
                frame_number,
                obj_id,
                x,
...
def load_image_paths(directory) -> List[str]:
    image_paths = []
...
def display_annotated_video(
    image_paths, obj_ids, org_meas, filtered_meas, smoothed_meas, model="Velocity'"
):
        ...
        for obj_id in obj_ids:
            try:
                ...
                raise MeasurementNotFoundException(
                    "Filtered x/y not found. Setting (0,0). Length orgmeas = "
                    f"{len(org_meas)}, index={idx2}"
                )
            ...
            try:
                ...
                raise MeasurementNotFoundException(
                    "Smoothed x/y not found. Setting (0,0). Length orgmeas = "
                    f"{len(org_meas)}, index={idx2}"
                )
            ...
            try:
                ...
                raise MeasurementNotFoundException(
                    "Original x/y not found. Setting (0,0). Length orgmeas = "
                    f"{len(org_meas)}, index={idx2}"
                )
            # try:
            ...
            smoothed_top_left = (
                int(smoothed_x - w_smoothed / 2),
                int(smoothed_y - h_smoothed / 2),
            )
            smoothed_bottom_right = (
                int(smoothed_x + w_smoothed / 2),
                int(smoothed_y + h_smoothed / 2),
            )
